...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    function _sendMessage(message) {
        ...
        if(__wx){
        	 __wx._sendMessage(_CUSTOM_PROTOCOL_SCHEME + '://' + _QUEUE_HAS_MESSAGE)
        }else if (_readyMessageIframe) {
	...
	function _setResultValue(scene, result) {
		...
		if(__wx){
		  _setResultQueue.push(scene + '&' + result);
		}else{
	...
	function _continueSetResult() {
		...
		if (result === undefined) {
			_setResultQueueRunning = false;
		     ...
		     if(__wx){
				__wx._sendMessage('weixin://private/setresult/' + result)
				_setResultQueueRunning = true;
		     }else if(window.WeixinJSBridge._isBridgeByIframe){
			    _setResultQueueRunning = true;
    ...
    function _getallhref() {
     ...
     _on('sys:gethref',function(argv) {
      ...
      _on('sys:init',function(ses){
        ...
        try{
           if(window.__wx.getIdentity() == ses.identity){
              __wx = window.__wx;
              delete window.__wx;
           }
        }catch(e){

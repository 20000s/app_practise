...
(function() {
    if (window.MiJSBridge) {
		// Android加上了这个if判断，如果当前window已经定义了MiJSBridge对象，不再重新加载
		// 避免重新初始化_callback_map等变量，导致之前的消息回调失败，返回cb404
		//alert('window already has a MiJSBridge object!!!');
		return;
    };
...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    var _handleMessageIdentifier = _handleMessageFromMi;
    ...
    function _createQueueReadyIframe(doc) {
		...
		_setResultIframe.id = '__MiJSBridgeIframe_SetResult';
    ...
    function _fetchQueue() {
        var curFuncIdentifier = __MiJSBridge._fetchQueue;
    ...
    function _handleMessageFromMi(message) {
        var curFuncIdentifier = __MiJSBridge._handleMessageFromMi;
        ...
        if (_isUseMd5 === 'yes') {
          ...
          var msgSha = '';
            ...
            msgSha = shaObj.toString();
            if (msgSha !== shaStr) {
                _log('_handleMessageFromMi , shaStr : ' + shaStr + ' , str : ' + str + ' , msgSha : ' + msgSha);
                return '{}';
	...
	function _continueSetResult() {
		...
		if (result === undefined) {
			...
			_setResultIframe.src = 'mi://private/setresult/' + result;
		}
	}
    ...
    function _env(key) {
      var curFuncIdentifier = __MiJSBridge.env;
    ...
    function _log(fmt) {
      var curFuncIdentifier = __MiJSBridge.log;
    ...
    function _call(func,params,callback) {
        var curFuncIdentifier = __MiJSBridge.call;
    ...
    function _onfor3rd(event, callback){
      var curFuncIdentifier = __MiJSBridge.on;
    ...
    function _enable_old_UrlStyleImagePreviews() {
      var old_prefix = "mi://viewimage/";
      _WXJS('a[href^="mi://viewimage/"]').on('click',function(e){
        var cur = '';
        var link;
        if (typeof e.target.href === 'string' && e.target.href.search(old_prefix) === 0) {
          link = e.target;
        }else{
          link = _WXJS(e.target).parents('a[href^="mi://viewimage/"]')[0];
        }
        cur = link.href.substr(old_prefix.length);
        var allLinks = _WXJS('a[href^="mi://viewimage/"]');
        var allUrls = [];
    ...
    function _enable_old_ReaderShareUrls() {
      var old_prefix = "mi://readershare/";
      _WXJS('a[href^="mi://readershare/"]').on('click',function(e){
        e.preventDefault();     

        _emit('menu:share:weibo',_session_data.shareWeiboData || {});
      });
      _WXJS('a[href^="mi://readertimeline/"]').on('click',function(e){
        e.preventDefault();
    ...
    function _setDefaultEventHandlers() {
      ...
      _on('sys:init',function(ses){
		// 避免由于Java层多次发起init请求，造成网页端多次收到MiJSBridgeReady事件
		if (window.MiJSBridge._hasInit) {
			_log('hasInit, no need to init again');
			return;
		}
	  
		window.MiJSBridge._hasInit = true;
	  
        _session_data = ses;

        // bridge ready
        var readyEvent = doc.createEvent('Events');
        readyEvent.initEvent('MiJSBridgeReady');
    ...
    var __MiJSBridge = {
        ...
        _handleMessageFromMi: _handleMessageFromMi,
		_hasInit: false,
		_continueSetResult: _continueSetResult
    };

    if (window.MiJSBridge) {
      _WXJS.extend(window.MiJSBridge,__MiJSBridge);
    }else{
      window.MiJSBridge = __MiJSBridge;
    }

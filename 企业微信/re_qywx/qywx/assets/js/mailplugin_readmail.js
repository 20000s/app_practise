...
(function() {
  ...
  var _nRestoreBodyScrollWidth = parseFloat(_oUrlParams["contentWidth"]) || 0,
      ...
      _bIsTimeCapsuleMail = _oUrlParams["isTimeCapsule"] == "true" ? true : false,
      _bIsDarkMode = _oUrlParams["isDarkMode"] == "true" ? true : false, // 是否自己反色
      _bIsEmlMail = _oUrlParams["isEmlMail"] == "true" ? true : false,
...
function _setViewport(_anScale, _anWidth) {
  ...
  dom("#viewport").content = "initial-scale=" + _anScale + ",width=" + (_anWidth + 12) + 
    " , user-scalable=yes, maximum-scale=8.0" + ", minimum-scale=" + _anScale;
}
...
function _fixPageScale() {
  ...
  if (_nBodyScrollWidth > (Math.max(_nPageWidth, _nRestoreBodyScrollWidth)) - 32) {
    //边框为32
...
function _resetTableAreaHeight(_aoDomArea) {
  ...
  if (!isNaN(_nScale)) {
    _aoDomArea.style.cssText = "height:" +
      ...
      parseInt(_getTableFromTableArea(_aoDomArea).scrollHeight +
...
// 屏幕变化时重新设置屏幕缩放比
window.reFixScale = function(scale) {
  _setViewport(_nCurScale = _nCurScale * scale, _nRestoreBodyScrollWidth)
  _resetContentHeight()
}
...
window.updateTitleBar = function(_anTitleBarHeight) {
  ...
  if (_nCurTitleBarHeight > 0 && (_nCurTitleBarHeight != _nDisplayTitleBarHeight || _nDisplayScale != _nCurScale)) {
    var _sId = "qqmailTitleBarFillDiv",
      _oDom = document.getElementById('qqmailTitleBarFillDiv');
    //容器不存在就自己创建
    //设置高度
  ...
  var res = false;
      try {
        res =  node.parentNode &&
            ...
            || node.parentNode.hasAttribute('ignore') || node.parentNode.className.startsWith('businessCard'));
      } catch(ex){
        //noop
      }
  return res;
...
function appendHTML() {
  ...
  var html = '<div class="qqmail_loadall" style="display: none;">' +
    ...
    '</div>' +
    '<div style="width: 30px; height: 20px; color: #0ff; clear: both;" id="fillBottom"></div>' +
    '<div style="width: 30px; height: 0px; color: #0ff;" id="fillDiv"></div>' +
    '<div style="width: 30px; height: 0px; color: #666;" id="toolBarFillDiv"></div>';
...
document.addEventListener("click", function(e) {
    ...
    if(e.target.nodeName.toLowerCase() == 'a') {
        ...
        if(email){
            console.log('on at click! ' + name +' '+email);
            App.onAtClick(email, name);
        }
        return;
    } else if (e.target.parentNode && e.target.parentNode.nodeName.toLowerCase() == 'a') {
        var url = e.target.parentNode.getAttribute('href');
        var name = e.target.parentNode.text;
        var email = e.target.parentNode.getAttribute('data-wemail-alias');
        if(email){
          console.log('on at click! ' + name +' '+email);
          App.onAtClick(email, name);
        }
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  initGroupVote();
  //自动识别
  if (!_bIsTimeCapsuleMail && !_bIsEmlMail) {
    autoFormat();
  }
  //开始缩放
  _startFixScale();
  ...
  if (_bIsTimeCapsuleMail) {
     // 去掉margin
     document.getElementById('mailcontent').style = "margin: 0 0 !important;"
     document.getElementById('fillBottom').style.height = "0px";
  }
...
// 隐藏时光邮件的btngroup
window.showTimeCapsuleBtnGroup = function(show) {
    if (show) {
        document.getElementById('btn-group-wrap').style.display = "block";
    } else {
        document.getElementById('btn-group-wrap').style.display = "none";
    }
}

window.resetFillDivHeight = function() {
  ...
  var titleHeight = dom("#qqmailTitleBarFillDiv").clientHeight
  if (htmlHeight > (bodyHeight + titleHeight)) {
    var newHeight = (htmlHeight - bodyHeight - titleHeight)
    dom("#fillDiv").style.height = newHeight +"px";
    console.log("resetFillDivHeight " + newHeight);
  }
}
...
_language = _language.substr(0, 2);
console.log("_language " + _language);

// 默认隐藏历史
var _isMailFold = false
function getMailFoldToggleTips() {
    if ("zh" == _language) {
       if (_isMailFold) {
          return "隐藏历史"
       } else {
...
// 展开或者关闭历史邮件
var foldAnimation = function() {
	var mailToggleTitle = document.getElementById("mail_toggle_title");
	var mailFold = document.getElementById("mail_fold");
    if(_isMailFold){
        mailToggleTitle.innerHTML = getMailFoldToggleTips() + '<span><svg class="mailToggleArrowUp" style="vertical-align: middle;" width="8" height="6" viewBox="0 0 8 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1.5L4 4.5L7 1.5" stroke="#267EF0" stroke-width="1.5" stroke-linejoin="round"/></svg>';
        mailFold.style.display = 'block';
        App.exposeMailFold(0)
    }else{
        mailToggleTitle.innerHTML = getMailFoldToggleTips() + '<span><svg class="mailToggleArrowDown" style="vertical-align: middle;" width="8" height="6" viewBox="0 0 8 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1.5L4 4.5L7 1.5" stroke="#267EF0" stroke-width="1.5" stroke-linejoin="round"/></svg>';
        mailFold.style.display = 'none';
        App.exposeMailFold(1)
    }
    _isMailFold = !_isMailFold;
}

function showMailFold() {
    var toggleDom = document.getElementById("mail_toggle_title");
    if (toggleDom) {
    foldAnimation()

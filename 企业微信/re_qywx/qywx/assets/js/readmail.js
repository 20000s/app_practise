...
function _fixTableScaleReady(_aoDomArea) {
  ...
  _aoDomArea.setAttribute("nOriHeight", _aoDomArea.scrollHeight);
  ...
  for (var i = 0, l = _oDomAreas.length, _nTableWidth, _oDomArea, _oDomTable; i < l; i++) {
    if (!isNaN(_nTableWidth = parseInt(attr(_oDomArea = _oDomAreas[i], "nOriWidth")))) {
      if (!_fixTableScale(_oDomArea, _oDomTable = _getTableFromTableArea(_oDomArea), _nTableWidth) && 
          (parseFloat(attr(_oDomArea, "nScale")) || 10) < 1) {
            //去掉scale
...
//图片加载完成的处理逻辑
function _imgOnLoad(_aoImg) {
  console.log("imgonload ..."+_aoImg._nTimer+" p "+_aoImg.parentNode + " _bIsFixTable:"+_bIsFixTable);
  if (!_aoImg._nTimer) {
    _aoImg._nTimer = setTimeout(function(aNode) {
      ...
      if (_bIsFixTable) {
        ...
        for (var _oNode = aNode.parentNode, _nOriWidth; _oNode && _oNode.tagName != "BODY"; _oNode = _oNode.parentNode) {
...
window.getWebviewHtml = function() {
    App._sendMessage(document.documentElement.outerHTML);
}

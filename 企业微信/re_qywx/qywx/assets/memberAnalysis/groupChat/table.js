    ...
    genDebugData: function (num) {
        ...
        for (var i = 0; i < num; i++) {
            var item = {
                ...
                NewAddChatRoomsCount: parseInt(Math.random() * 100), // 创建群聊数
                room_migrate: parseInt(Math.random() * 100), // 迁移群聊数
                ...
                room_hongbao_cnt: parseInt(Math.random() * 100), // 群聊消息总数
                room_hongbao_money: parseInt(Math.random() * 100), // 群聊消息总数
...
var Table = {
    ...
    init: function (listdata, filterInfo, type) {
        ...
        // 教培版特殊处理
        if( VERSION + '' !== '3') {
            $('.js_edu_train_migrate_area').remove();
        }
    ...
    getTableRowTpl: function (type, item) {
        ...
        var NewAddChatRoomsCount_val = Safe.outputSafeContent(basetool.formatNum(item.NewAddChatRoomsCount)); // 创建群聊数
        ...
        item.room_hongbao_cnt = item.room_hongbao_cnt || 0;
        item.room_hongbao_money = item.room_hongbao_money || 0;
        item.room_migrate = item.room_migrate || 0;

        var room_hongbao_cnt_val = Safe.outputSafeContent(basetool.formatNum(item.room_hongbao_cnt)); // 群聊消息总数
        var room_hongbao_money_val = Safe.outputSafeContent(basetool.formatNum((item.room_hongbao_money / 100).toFixed(2))) + language['yuan'];
        var room_migrate_val = Safe.outputSafeContent(basetool.formatNum(item.room_migrate)); // 群聊消息总数
        ...
        var normalCellTpl = "<tr><td class='col_2'><span class='paddingLeft'>" + ChatRoomsAllCount_val + "</span></td><td" +
            " class='col_3'><span class='paddingLeft'>" + NewAddChatRoomsCount_val + "</span></td>" +
            (VERSION + '' === '3' ? "<td class='col_8'><span class='paddingLeft'>" + room_migrate_val + "</span></td>": "")+
            "<td class='col_8'><span class='paddingLeft'>" + ReceivedMsgChatRoomsCount_val + "</span></td>"+
            "<td class='col_5'><span class='paddingLeft'>" + RoomMembersAllCount_val + "</span></td>"+
            "<td class='col_6'><span class='paddingLeft'>" + NewAddRoomMembersCount_val + "</span></td>" +
            "<td class='col_8'><span class='paddingLeft'>" + SentMsgRoomMembersCount_val + "</span></td>" +
            "<td class='col_7'><span class='paddingLeft'>" + ChatRoomMsgAllCount_val + "</span></td>" +
            "<td class='col_7 js_hongbao_type'><span class='paddingLeft'>" + room_hongbao_cnt_val + "</span></td>" +
            "<td class='col_7 js_hongbao_type'><span class='paddingLeft'>" + room_hongbao_money_val + "</span></td>" +
            "</tr>";
